...
export function ManualClientModal({ isOpen, onClose, onSuccess, initialDate }: ManualClientModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('clientAddedSuccessfully2'))
      onSuccess()
      onClose()
      setFormData({ client_name: '', contact: '', notes: '', session_date: '' })
    } catch (err: any) {
      toast.error(err.message || t('errorAddingClient'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-white dark:bg-neutral-900 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-center p-6 border-b border-neutral-100 dark:border-white/5">
          <h2 className="text-xl font-bold text-neutral-900 dark:text-white">{t('addAClientManually')}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">{t('clientName')}</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder={t('ivanIvanov')}
              />
            </div>
          </div>
          
          <div>
            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">{t('tgInst')}</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-neutral-800 rounded-xl pl-10 pr-4 py-3 text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500"
                placeholder={t('username123456789')}
              />
            </div>
          </div>
          
          <div>
            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">{t('notesIdea')}</label>
            <div className="relative">
          ...
          <div>
            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">{t('sessionDateOptional')}</label>
            <div className="relative">
