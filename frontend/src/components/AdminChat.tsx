...
export function AdminChat() {
  ...
  const handleToggleBan = (userId: string) => {
    ...
    setConfirmModal({
      isOpen: true,
      title: isBanned ? t('key_ea8259') : t('key_30a06e'),
      message: `Вы уверены, что хотите ${isBanned ? t('key_c1a096') : t('key_e3d581')} пользователя ${user.email}?`,
      confirmText: isBanned ? t('unlock') : t('key_35903d'),
      cancelText: t('cancel'),
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          toast.success(isBanned ? t('key_985991') : t('key_3662d0'))
          ...
          {isLoadingChats ? (
            ...
            chatList.map(c => (
              <button
                ...
                className={`w-full text-left p-4 border-b border-neutral-100 dark:border-neutral-800/50 hover:bg-white dark:hover:bg-neutral-800 transition-colors ${selectedUserId === c.id ? 'bg-white dark:bg-neutral-800 border-l-4 border-l-accent-500' : ''} ${c.unread > 0 && selectedUserId !== c.id ? 'bg-accent-50/50 dark:bg-accent-900/10' : ''}`}
              >
                <div className="flex justify-between items-center mb-1">
                  ...
                  {c.unread > 0 && selectedUserId !== c.id && (
                    <div className="w-5 h-5 bg-accent-500 text-white rounded-full flex items-center justify-center text-[10px] font-bold">
                      {c.unread}
                    </div>
                  )}
                </div>
          ...
          <>
            <div className="p-4 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between shadow-sm z-10 bg-white dark:bg-neutral-900">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  title={usersMap[selectedUserId]?.status === 'rejected' ? t('unlock') : t('key_35903d')}
                >
                    ...
                    <div key={msg.id} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                      <div className={`flex flex-col ${isAdmin ? 'items-end' : 'items-start'} max-w-[70%]`}>
                        ...
                        >
                          ...
                          <div className={`text-[10px] leading-none flex items-center justify-end gap-0.5 whitespace-nowrap ${isAdmin ? 'text-accent-100' : 'text-neutral-400'}`}>
            ...
            <form onSubmit={handleSendMessage} className="p-4 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-50 dark:bg-neutral-900 flex gap-2">
              ...
              <button
                ...
                className="bg-accent-500 hover:bg-accent-600 text-white px-4 py-2 rounded-xl disabled:opacity-50 transition-colors flex items-center justify-center"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200" onClick={(e) => { if (e.target === e.currentTarget) setBalanceModalUser(null) }}>
          <div className="bg-white dark:bg-neutral-900 w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in zoom-in-95 duration-200 p-6 border border-neutral-200 dark:border-neutral-800">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-neutral-900 dark:text-white">{t('key_f5116f')}</h3>
              <button 
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium text-neutral-600 dark:text-neutral-400 mb-2">{t('key_27f502')}</label>
              <input
                type="number"
                min="0"
