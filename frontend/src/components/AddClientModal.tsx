...
export function AddClientModal({ isOpen, onClose, onSuccess, onDuplicateFound }: AddClientModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
            ...
            toast((toastInstance) => (
              <div className="flex flex-col gap-2">
                <span className="font-medium text-sm">
                  {t('aClientWithThese')} <b>{errData.detail.client.name}</b>
                </span>
                <button 
                  onClick={() => {
      ...
      toast.success(t('clientAddedSuccessfully'))
      onSuccess()
      onClose()
    } catch (err) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-white dark:bg-neutral-900 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden my-8">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('name')}</label>
            <input
              type="text"
              required
              value={formData.name}
          ...
          <div>
            <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('notes')}</label>
            <textarea
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            >
              {loading ? t('creation') : t('addAClient')}
            </button>
          </div>
        </form>
      </div>
