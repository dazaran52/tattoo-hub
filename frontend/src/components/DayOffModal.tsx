...
export function DayOffModal({ isOpen, onClose, onSuccess }: DayOffModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('dayOffSuccessfullyAdded'))
      onSuccess()
      onClose()
      setSessionDate('')
    } catch (err: any) {
      toast.error(err.message || t('errorAddingHoliday'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-white dark:bg-neutral-900 rounded-3xl w-full max-w-sm shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        <div className="flex justify-between items-center p-6 border-b border-neutral-100 dark:border-white/5 bg-red-50 dark:bg-red-900/10">
          <h2 className="text-xl font-bold text-red-600 dark:text-red-400">{t('addADayOff')}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-semibold text-neutral-700 dark:text-neutral-300 mb-1">{t('dayOffDate')}</label>
