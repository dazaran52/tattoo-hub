...
export function SessionsList({ sessions, searchQuery, setSearchQuery, onStatusChange, onSessionClick, onUpdate, cardView = 'normal' }: SessionsListProps) {
  ...
  const handleBulkStatusChange = async (newStatus: string) => {
    try {
      ...
      toast.error(t('errorUpdatingStatuses'))
    }
  }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-neutral-900 p-4 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <option value="discussing">{t('inDialogue')}</option>
            <option value="booked">{t('recorded')}</option>
          ...
          <div className="text-sm text-neutral-500 font-medium">
            {t('found')} {filteredAndSorted.length}
          </div>
        </div>
            ...
            <tbody>
                  ...
                  <td className="p-4">
                    ...
                    {cardView === 'expanded' && (
                      <div className="text-[10px] text-neutral-400 mt-1">
                        {t('createdBy')} {new Date(session.created_at).toLocaleDateString('ru-RU')}
                      </div>
                    )}
                  </td>
                  <td className="p-4">
              ...
              {paginated.length === 0 && (
                <EmptyState
                  ...
                  description={searchQuery ? t('thereAreNoSessions') : t('youDonTHave3')}
                />
              )}
            </tbody>
          </table>
