...
import { RefreshCw, Search, Loader2, Plus, Edit2, Trash2, XCircle, ChevronLeft, ChevronRight, Image as ImageIcon, Clock, Bell, BellOff, MapPin, Calendar, Palette, Maximize2, Wallet, PersonStanding, Scale3d, DollarSign } from 'lucide-react'
...
export function LeadsFeed({ onUnlockSuccess, isAdmin = false, isMarketplace = false, showOnlyUnlocked = false, userCities = [] }: LeadsFeedProps) {
  ...
  const togglePushNotifications = async () => {
    if (!('Notification' in window)) {
      toast.error(t('browserDoesnTSupport'))
      return
    }
    if (Notification.permission === 'denied') {
      toast.error(t('notificationsAreBlockedIn'))
      return
    }
    if (pushEnabled) {
      setPushEnabled(false)
      toast.success(t('notificationsAreDisabled'))
    } else {
      const permission = await Notification.requestPermission()
      if (permission === 'granted') {
        setPushEnabled(true)
        toast.success(t('newLeadNotificationsEnabled'))
      } else {
        toast.error(t('notificationAccessDenied'))
      }
    }
  }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime_leads')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'leads' }, (payload) => {
        ...
        toast(t('updatingTheLeadDatabase'), { icon: '🔄', duration: 2000 })
        fetchLeads(true)
      })
      .subscribe()
  ...
  const handleAcceptB2BProposal = async (leadId: string, masterId: string) => {
    ...
    try {
      ...
      toast.success(t('theLeadWasSuccessfully2'))
      playSuccessSound()
      triggerHaptic('success')
      fetchLeads()
    } catch (err: any) {
      toast.error(err.message || t('errorAcceptingOffer'))
      playErrorSound()
    } finally {
  ...
  const handleDeleteLead = async (leadId: string) => {
    ...
    const leadTitle = lead ? lead.title : t('thisLead')
            ...
            <div className="p-4 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="budget_desc">{t('dearOnesFirst')}</option>
                  <option value="budget_asc">{t('inexpensiveFirst')}</option>
                </select>
              </div>

              <div>
                <label className="block text-xs font-bold text-neutral-500 mb-2 uppercase">{t('styles')}{selectedStyles.length})</label>
                <select
                  className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-neutral-900 dark:text-white"
                  onChange={(e) => {
              ...
              <div>
                <label className="block text-xs font-bold text-neutral-500 mb-2 uppercase">{t('crmBoard.sizeLabel')}</label>
                <select
                  className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-neutral-900 dark:text-white"
                  onChange={(e) => {
                ...
                >
                  ...
                  <option value={t('sleeveFullBack')}>{t('sleeveFullBack')}</option>
                </select>
              ...
              <div className="lg:col-span-4 md:col-span-2 flex items-center justify-between p-3 bg-neutral-50 dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700">
                <div className="flex flex-col">
                  <span className="text-sm font-bold text-neutral-900 dark:text-white">{t('onlyFromOnlineClients')}</span>
                  <span className="text-xs text-neutral-500 dark:text-neutral-400">{t('showRequestsOnlyFrom')}</span>
                </div>
      ...
      {leads.length > 0 && filteredLeads.length === 0 && !error && (
        <div className="text-center p-12 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800">
          ...
          <p className="text-neutral-500 dark:text-neutral-400 text-sm">
            {showOnlyUnlocked ? t('youHavenTPurchased') : t('thereAreNoLeads')}
          </p>
        </div>
      )}
                ...
                <div className="flex flex-col gap-1 min-w-0 mb-4">
                  ...
                  {lead.created_at && (
                    <span className="text-[10px] text-neutral-400 font-medium ml-1">
                      {t('createdBy')} {new Date(lead.created_at).toLocaleDateString('ru-RU', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute:'2-digit' })}
                    </span>
                  )}
                </div>
              ...
              <div className="p-5 border-t border-neutral-100 dark:border-neutral-800/80 bg-neutral-50/50 dark:bg-neutral-900/50 backdrop-blur-sm">
                {viewMode === 'my-shared' ? (
                  <div className="space-y-4">
                    <h4 className="font-bold text-neutral-900 dark:text-white mb-2">{t('responsesFromTheMasters')}{lead.proposals?.length || 0})</h4>
                          ...
                          <div key={prop.id} className="p-4 bg-white dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-xl">
                            <div className="flex justify-between items-start mb-3">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-bold text-sm dark:text-white">{prop.users?.full_name || t('master')}</p>
                                  <p className="text-xs text-neutral-500">{t('offersPerSession')} <span className="font-bold text-accent-600">{prop.price_offer} CZK</span></p>
                                </div>
                              </div>
                            </div>
                            {prop.status === 'accepted' ? (
                              <div className="w-full py-2 px-3 bg-green-50 text-green-700 text-sm font-bold text-center rounded-lg">{t('youHaveChosenThis')}</div>
                            ) : prop.status === 'pending' ? (
                              <div className="flex gap-2">
                                ...
                                >
                                  {actionLoadingId === `${lead.id}-${prop.user_id}` ? t('acceptance') : `Отдать (ваш заработок: ${prop.price_offer * 0.08} CZK)`}
                                </button>
                              </div>
                            ) : (
                              <div className="w-full py-2 px-3 bg-red-50 text-red-700 text-sm font-bold text-center rounded-lg">{t('rejected')}</div>
                            )}
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-sm text-neutral-500 text-center py-4">{t('thereAreNoOffers')}</p>
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="w-full py-3 px-4 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-900/50 rounded-xl text-sm font-bold shadow-inner flex items-center justify-center gap-2">
                      ...
                      {isAdmin ? "ADMIN VIEW" : t('wonTheClientChose')}
                    </div>
                    {lead.chat_id && (
                      <button
