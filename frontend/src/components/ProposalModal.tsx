...
export function ProposalModal({ isOpen, onClose, lead, onSuccess }: ProposalModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (price <= 0 || (!hasDates && !formData.proposed_dates.trim())) {
      toast.error(t('specifyPriceAndAvailable'))
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (responseData.detail === 'MAX_PROPOSALS_REACHED') {
          throw new Error(t('57'))
        }
        if (responseData.detail === 'INSUFFICIENT_BALANCE_FOR_COMMISSION') {
          throw new Error(t('thereAreNotEnough'))
        }
        throw new Error(responseData.detail || t('failedToSubmitProposal'))
      }

      if (responseData.chat_id) {
        toast.success(t('theApplicationHasBeen2'))
        window.location.href = `/dashboard?tab=messages&chat_id=${responseData.chat_id}`
      } else {
        toast.success(t('offerSentFreeOf'))
        onSuccess()
      }
    } catch (error) {
      ...
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm" onClick={(event) => event.target === event.currentTarget && onClose()}>
        ...
        >
          <div className="flex shrink-0 items-center justify-between border-b border-neutral-100 p-4 dark:border-white/5 lg:p-6">
            <h2 className="text-xl font-bold text-neutral-900 dark:text-white">{t('makeAnOffer')}</h2>
            <button type="button" onClick={onClose} aria-label={t('close')} className="rounded-full bg-neutral-100 p-2 text-neutral-500 dark:bg-neutral-800">
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-1 flex-col min-h-0">
            ...
            <div className="flex items-start gap-3 rounded-2xl bg-primary-500/10 p-4 text-sm text-primary-600 dark:text-primary-400">
              ...
              <p>{t('shippingIsFreeThe')}</p>
            </div>

            {lead.display_budget && (
              <div className="rounded-2xl bg-emerald-500/10 p-3 text-sm font-bold text-emerald-700 dark:text-emerald-300">
                {t('clientBudget')} {lead.display_budget}
              </div>
            )}

            <div>
              <label className="mb-2 block text-sm font-bold text-neutral-700 dark:text-neutral-300">{t('yourApproximatePrice')}{currency})</label>
              <input
                type="text"
                inputMode="numeric"
                pattern="\d*"
            ...
            {price > 0 && (
              <div className="rounded-2xl border border-neutral-200 p-4 text-sm dark:border-neutral-700">
                <div className="flex justify-between"><span>{t('tattooHub2')}</span><strong>{feeRate * 100}%</strong></div>
                <div className="mt-2 flex justify-between text-base"><span>{t('willBeWrittenOff')}</span><strong>{feeAmount} {currency}</strong></div>
                <div className="mt-2 flex justify-between text-xs text-neutral-500">
                  <span>{t('yourBalance2')}</span>
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <label className="block text-sm font-bold text-neutral-700 dark:text-neutral-300">{t('availableDates')}</label>
                <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-neutral-500">
                  ...
                  <div className="flex items-center gap-1"><div className="h-1.5 w-1.5 rounded-full bg-red-500"></div>{t('weekend')}</div>
                  {lead.session_date && <div className="flex items-center gap-1 text-purple-600 dark:text-purple-400"><div className="h-1.5 w-1.5 rounded-full bg-purple-500"></div>{t('clientChoice')}</div>}
                </div>
              </div>
              ...
              <label className="mb-2 block text-sm font-bold text-neutral-700 dark:text-neutral-300">{t('commentOnTheApplication')}</label>
              <textarea
                ...
                placeholder={t('writeToTheClient2')}
                rows={2}
            ...
            <div className="shrink-0 border-t border-neutral-100 p-4 dark:border-white/5 lg:p-6 bg-white dark:bg-neutral-900">
              <button type="submit" disabled={loading || isBalanceInsufficient} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-primary-500 py-4 font-bold text-white shadow-lg shadow-primary-500/25 hover:bg-primary-600 disabled:opacity-50">
                {loading ? <div className="h-6 w-6 animate-spin rounded-full border-2 border-white/20 border-t-white" /> : <><span>{isBalanceInsufficient ? t('insufficientFunds') : t('sendForFree')}</span><Send className="h-5 w-5" /></>}
              </button>
            </div>
          </form>
        </motion.div>
