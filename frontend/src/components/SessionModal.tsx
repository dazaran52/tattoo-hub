...
export function SessionModal({ isOpen, onClose, onSuccess, initialDate, initialClientId, existingClients, editSession, onSellLead }: SessionModalProps) {
  ...
  const handleCancelSession = async () => {
    if (!editSession) return
    if (!window.confirm(t('areYouSureYou5'))) return
    ...
    try {
      ...
      toast.success(t('sessionCanceled'))
      onSuccess()
      onClose()
    } catch (e: any) {
      toast.error(e.message || t('sessionCancellationError'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
          ...
          if (!clientRes.ok) {
            if (clientRes.status === 409) {
              ...
              if (errData?.detail?.error === 'client_exists') {
                ...
                throw new Error(t('errorCreatingClient'))
              }
            } else {
              throw new Error(t('errorCreatingClient'))
            }
      ...
      toast.success(editSession ? t('sessionUpdated') : t('sessionSuccessfullyCreated'))
      onSuccess()
      onClose()
    } catch (error: any) {
      console.error(error)
      toast.error(error.message || t('anErrorHasOccurred'))
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm overflow-y-auto" onClick={(e) => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bg-white dark:bg-neutral-900 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden my-8 flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b border-neutral-200 dark:border-neutral-800 shrink-0">
          <h2 className="text-xl font-bold flex items-center gap-2 text-neutral-900 dark:text-white">
            ...
            {editSession ? t('editSession') : t('createSession')}
          </h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <div className="space-y-4 bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-2xl border border-neutral-200 dark:border-neutral-800">
            <div className="flex items-center justify-between">
              <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">{t('client')}</label>
              {!isClientLocked && (
                <button
                  type="button"
                ...
                >
                  {isNewClient ? t('selectExisting') : <><Plus className="w-3 h-3"/> {t('addNew')}</>}
                </button>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('sessionDate')}</label>
              <input
                type="date"
                required
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('start')}</label>
                <input
                  type="time"
              ...
              <div>
                <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('end')}</label>
                <input
                  type="time"
            ...
            <div>
              ...
              <div className="flex flex-col gap-3">
                ...
                <input
                  ...
                  placeholder={t('clarificationEgInnerSide')}
                  className="w-full bg-neutral-100 dark:bg-neutral-800 border-none rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-accent-500/20 outline-none"
                />
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('photoReferences')}</label>
              <div className="flex flex-wrap gap-3">
                ...
                <label className="w-16 h-16 rounded-lg border-2 border-dashed border-neutral-300 dark:border-neutral-700 flex flex-col items-center justify-center text-neutral-500 hover:text-accent-500 hover:border-accent-500 transition-colors cursor-pointer bg-neutral-50 dark:bg-neutral-800/50">
                  ...
                  <span className="text-[9px] font-medium uppercase tracking-wider">{t('add')}</span>
                  <input 
                    type="file" 
                    accept="image/*" 
                    multiple 
            ...
            {editSession && editSession.status === 'completed' && (
              <div>
                <label className="block text-xs font-semibold text-neutral-500 dark:text-neutral-400 mb-1.5 uppercase tracking-wider">{t('photoOfTheResult')}</label>
              <div className="flex flex-wrap gap-3">
          ...
          <div className="p-6 border-t border-neutral-200 dark:border-neutral-800 shrink-0 flex gap-3 bg-white dark:bg-neutral-900">
            ...
            >
              {(loading || isUploading) ? (
                ...
                editSession ? t('saveChanges') : t('saveSession')
              )}
            </button>
          </div>
        </form>
