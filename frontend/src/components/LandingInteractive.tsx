          ...
          >
            ...
            >
              ...
              <motion.div 
                ...
                className="absolute inset-0 opacity-40 group-hover:opacity-80 transition-opacity duration-700 pointer-events-none scale-110"
              >
                <div className="absolute top-[20%] left-[20%] w-[60%] h-[60%] rounded-full bg-primary-600/30 blur-[100px]" />
                <div className="absolute bottom-[20%] right-[20%] w-[50%] h-[50%] rounded-full bg-primary-600/30 blur-[100px]" />
              </motion.div>
              <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:40px_40px] z-0 opacity-50 pointer-events-none" />
              
              <div className="relative z-10 flex flex-col items-center text-center">
                <div className="w-24 h-24 rounded-full bg-primary-950 border border-primary-900 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 shadow-2xl shadow-primary-900/50">
                  <UserCircle2 className="w-12 h-12 text-primary-400 group-hover:text-primary-300 transition-colors" />
                </div>
                <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4 text-neutral-300 group-hover:text-white transition-colors">
          ...
          >
            ...
            >
              {role === 'master' ? (
                 ...
                 <div className="absolute top-[20%] left-[20%] w-[60%] h-[60%] rounded-full bg-primary-600/20 blur-[120px]" />
              )}
            </motion.div>
            ...
            <div className="relative z-10 max-w-md w-full flex flex-col items-center text-center pb-24 md:pb-0">
              ...
              {guideSlide < currentSlides.length ? (
                    ...
                    <div className="mt-12 flex flex-col items-center gap-6 w-full">
                      <div className="flex gap-2">
                        {currentSlides.map((_, i) => (
                          <div key={i} className={`h-2 rounded-full transition-all duration-300 ${i === guideSlide ? (role === 'master' ? 'w-8 bg-orange-500' : 'w-8 bg-primary-500') : 'w-2 bg-neutral-800'}`} />
                        ))}
                      </div>
                      <button
                        onClick={handleNextGuide}
                        className={`w-full py-4 rounded-full font-bold text-lg hover:scale-105 transition-transform ${role === 'master' ? 'bg-orange-600 hover:bg-orange-500 text-white' : 'bg-primary-600 hover:bg-primary-500 text-white'}`}
                      >
                ...
                >
                  <div className="mb-8">
                    {role === 'master' ? <Brush className="w-16 h-16 text-orange-400 mx-auto" /> : <UserCircle2 className="w-16 h-16 text-primary-400 mx-auto" />}
                  </div>
                  
                  {role === 'client' && (
                    <button 
                      onClick={() => router.push('/new-lead')}
                      className="w-full flex items-center justify-center gap-2 px-6 py-5 md:py-4 rounded-full bg-gradient-to-r from-primary-500 to-primary-500 hover:from-primary-400 hover:to-primary-400 text-white font-bold text-lg transition-all shadow-lg shadow-primary-500/25"
                    >
