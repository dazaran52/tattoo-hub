    ...
    return (
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
        ...
        <button
          ...
          aria-label={isOpen ? t('key_ffd2db') : t('key_3a770a')}
    ...
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
      <AnimatePresence>
        ...
        <div className="p-4 bg-gradient-to-r from-accent-500 to-blue-500 text-white flex justify-between items-center shrink-0">
          ...
          <button onClick={() => setIsOpen(false)} aria-label={t('key_ffd2db')} className="hover:bg-white/20 p-1 rounded-lg transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
                ...
                <div key={msg.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                  <div className={`flex flex-col ${isMine ? 'items-end' : 'items-start'} max-w-[85%]`}>
                    ...
                    >
                      ...
                      <div className={`text-[10px] leading-none flex items-center justify-end gap-0.5 whitespace-nowrap ${isMine ? 'text-accent-100' : 'text-neutral-400'}`}>
        ...
        <form onSubmit={handleSendMessage} className="p-3 bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-800 flex gap-2 shrink-0">
          <input
            ...
            className="flex-1 bg-neutral-100 dark:bg-neutral-800 border-transparent focus:border-accent-500 focus:ring-1 focus:ring-accent-500 rounded-xl px-4 py-2 text-sm text-neutral-900 dark:text-white outline-none transition-all"
          />
          <button
            type="submit"
            disabled={!newMessage.trim() || isSending}
            aria-label={t('key_d25719')}
