...
export function Header({ profile, onLogout, maxWidthClass = 'max-w-7xl', onOpenCommandPalette }: HeaderProps) {
  ...
  const handleSubscribe = async () => {
    try {
      ...
      toast.success(t('notificationsAreSuccessfullyEnabled'))
    } catch (err: any) {
      if (Notification.permission === 'denied') {
        toast.error(t('youHaveBlockedNotifications'))
      } else {
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-4 md:gap-6">
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <button 
                onClick={toggleTheme} 
                className="w-[34px] h-[34px] rounded-xl flex items-center justify-center text-neutral-500 hover:text-neutral-900 dark:text-neutral-400 dark:hover:text-white bg-neutral-100/80 hover:bg-neutral-200 dark:bg-neutral-800/80 dark:hover:bg-neutral-700 border border-neutral-200 dark:border-neutral-700 transition-colors"
                title={t('theme')}
              >
                      ...
                      <>
                        ...
                        <div className="px-4 py-2 flex items-center justify-between group cursor-pointer hover:bg-neutral-100 dark:hover:bg-neutral-800"
                             onClick={() => {
                               ...
                               import('react-hot-toast').then(mod => mod.default.success(t('referralCopied')));
                             }}
                             title={t('copyReferralCode')}>
                           <div className="flex flex-col">
                             <span className="text-xs text-neutral-500">{t('referralCode')}</span>
