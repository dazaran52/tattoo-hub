...
export default function ClientPortalPage() {
  ...
  const sendMessage = async (e: React.FormEvent) => {
    e.preventDefault()
    toast.error(t('signInToSend'))
    window.location.href = '/login'
  }

  const acceptProposal = async (masterId: string) => {
    toast.error(t('theChoiceOfWizard'))
    window.location.href = '/login'
  }
      ...
      <div className="max-w-4xl mx-auto p-4 lg:p-8 space-y-8">
        ...
        <div className="bg-white dark:bg-neutral-800 rounded-3xl p-6 shadow-sm border border-neutral-200 dark:border-white/5">
          ...
          <div className="mt-4 inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 text-sm font-bold">
            {leadData.client_priority === 'cheap' ? t('lowPriceIsImportant') :
             leadData.client_priority === 'fast' ? t('doItAsSoon') : t('qualityIsMostImportant')}
          </div>
        </div>
        ...
        {isAnyAccepted && (
          <div className="bg-green-500/10 border border-green-500/20 p-4 rounded-3xl text-green-700 dark:text-green-400 font-bold flex items-center gap-3">
            ...
            {t('youHaveAlreadyChosen')}
                                </div>
        )}

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <h2 className="text-xl font-bold text-neutral-900 dark:text-white">{t('responsesFromTheMasters')}{leadData.proposals.length})</h2>
            ...
            {leadData.proposals.map(p => (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-neutral-500">
                        {isSelectedStatus(p.status) && <span className="text-green-500">{t('selected3')}</span>}
                        {p.status === 'pending' && <span className="text-amber-500">{t('waiting')}</span>}
                        {p.status === 'rejected' && <span className="text-red-500">{t('rejected3')}</span>}
                      </div>
                    </div>
                  </div>
                ...
                <div className="text-sm text-neutral-600 dark:text-neutral-400 bg-neutral-100 dark:bg-neutral-900 p-3 rounded-xl mb-3">
                  <span className="font-bold block mb-1">{t('availableDates2')}</span>
                  {p.proposed_dates}
                </div>
                ...
                {!isAnyAccepted && p.status === 'pending' && (
                  ...
                  >
                    <Check className="w-4 h-4" /> {t('selectAMaster')}
                                              </button>
                )}
              </div>
            ))}
              ...
              <div className="bg-white dark:bg-neutral-800 rounded-3xl border border-neutral-200 dark:border-white/5 flex flex-col h-[600px] overflow-hidden sticky top-8">
                <div className="p-4 border-b border-neutral-100 dark:border-white/5 bg-neutral-50/50 dark:bg-neutral-900/50 flex justify-between items-center">
                  <h3 className="font-bold text-neutral-900 dark:text-white flex items-center gap-2">
                    ...
                    {t('chatWith')} {selectedProposal.master_name}
                  </h3>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  ...
                  {chatLoading ? (
                    <div className="text-center text-neutral-400 py-4">{t('loadingMessages')}</div>
                  ) : messages.length === 0 ? (
