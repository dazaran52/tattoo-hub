...
export default function ReviewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!session) throw new Error(t('logInToLeave'))
      ...
      if (!res.ok) {
        ...
        throw new Error(data.detail || t('errorSendingFeedback'))
      }

      setIsSuccess(true)
    } catch (err) {
      setError(err instanceof Error ? err.message : t('anErrorHasOccurred'))
    } finally {
    ...
    return (
      <div className="min-h-screen bg-neutral-50 dark:bg-[#050505] flex flex-col items-center justify-center p-4">
        <div className="bg-white dark:bg-neutral-900 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl border border-neutral-200 dark:border-neutral-800">
          ...
          <h1 className="text-2xl font-bold text-neutral-900 dark:text-white mb-2">{t('thanksForYourFeedback')}</h1>
          <p className="text-neutral-500 dark:text-neutral-400 mb-8">
            {t('yourFeedbackWillHelp')}
                              </p>
          <button 
            onClick={() => router.push('/')}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-semibold opacity-90 mb-2 dark:text-white">
              {t('yourCommentOptional')}
                                      </label>
            <textarea
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder={t('everythingWentGreatSuper')}
              rows={4}
