...
export default function OnboardingPage() {
  ...
  const handleFinalSubmit = (e: React.FormEvent) => {
    ...
    if (profile?.role === 'master' && !portfolioUrl.trim()) {
      toast.error(t('pleaseProvideALink'))
      return
    }
    submitProfile()
  }
  ...
  const submitProfile = async () => {
    ...
    try {
      ...
      toast.success(t('theProfileHasBeen'))
      setIsSuccess(true)
    } catch (err: any) {
      toast.error(err.message || t('anErrorHasOccurred'))
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="relative w-full md:w-1/2 min-h-[30vh] md:min-h-screen flex flex-col items-center justify-center p-8 lg:p-16 overflow-hidden border-b md:border-b-0 md:border-r border-neutral-200/50 dark:border-white/5 bg-neutral-100 dark:bg-[#0a0a0a] z-0">
        ...
        <div className="relative z-10 flex flex-col items-center md:items-start text-center md:text-left w-full max-w-lg space-y-4">
          ...
          <p className="text-lg md:text-xl text-neutral-600 dark:text-neutral-300 font-light">
            {t('thereAreOnlyA')} {profile?.role === 'master' ? t('startGettingClients') : t('findYourIdealTattoo')}.
          </p>
        </div>
      </div>
          ...
          <AnimatePresence mode="wait">
            {isSuccess ? (
              ...
              >
                ...
                >
                  {profile?.role === 'master' ? t('crm6') : t('home')} <ArrowRight className="w-5 h-5" />
                </button>
              </motion.div>
            ) : (
              <motion.div 
              ...
              >
                ...
                <div className="mb-8">
                  <h2 className="text-2xl font-bold dark:text-white mb-2">
                    {step === 1 ? t('generalInformation') : t('confirmationOfQualifications')}
                  </h2>
                  <p className="text-sm text-neutral-500">
                    {step === 1 ? t('tellUsALittle') : t('addALinkTo')}
                  </p>
                </div>
                ...
                {step === 1 ? (
                  <form onSubmit={handleNextStep} className="space-y-5">
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 ml-1">
                        {profile?.role === 'master' ? t('yourNameStudioName') : t('leadWizard.yourNameLabel')}
                      </label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 ml-1">{t('leadWizard.countryLabel')}</label>
                        <div className="relative group">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 ml-1">{t('leadWizard.cityLabel')}</label>
                        <div className="relative group">
                          ...
                          <select
                            ...
                            className={`w-full pl-12 pr-4 py-4 bg-white/50 dark:bg-neutral-950/50 border border-neutral-200 dark:border-white/10 rounded-2xl focus:outline-none focus:ring-2 appearance-none cursor-pointer transition-all ${profile?.role === 'master' ? 'focus:ring-orange-500/20 focus:border-orange-500' : 'focus:ring-primary-500/20 focus:border-primary-500'}`}
                          >
                            <option value="" disabled>{t('leadWizard.selectCity')}</option>
                            {cities.map(c => (
                              <option key={c.id} value={c.id}>{c.name_ru}</option>
                            ))}
                          </select>
                    ...
                    >
                      {totalSteps === 1 ? t('complete') : t('guide.next')} <ArrowRight className="w-5 h-5" />
                    </button>
                  </form>
                ) : (
                  <form onSubmit={handleFinalSubmit} className="space-y-5">
                    <div className="space-y-1.5">
                      <label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300 ml-1">
                        {t('linkToPortfolio')} <span className="text-red-500">*</span>
                      </label>
                      <div className="relative group">
                    ...
                    <div className="flex gap-3 mt-8">
                      ...
                      >
                        {isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : t('completeRegistration')}
                      </button>
                    </div>
                  </form>
                )}
